...
class Ship {
  ...
  shoot() {
    ...
    let bullet6 = new defaultCurveBullet(0, -20, this.stage, this.game.ticker, -6);
  ...
  playerAction() {
    if (this.keys[37]) this.ship.x -= 5;
    if (this.keys[38]) this.ship.y -= 5;
    if (this.keys[39]) this.ship.x += 5;
    if (this.keys[40]) this.ship.y += 5;
    if (this.keys[90] &&
