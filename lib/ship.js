...
class Ship {
  ...
  shoot() {
    ...
    let bullet5 = new defaultCurveBullet(0, -20, this.stage, this.game.ticker, 10);
    let bullet6 = new defaultCurveBullet(0, -20, this.stage, this.game.ticker, -5.7);
  ...
  playerAction() {
    if (this.keys[37]) this.ship.x -= 4;
    if (this.keys[38]) this.ship.y -= 4;
    if (this.keys[39]) this.ship.x += 4;
    if (this.keys[40]) this.ship.y += 4;
    if (this.keys[90] &&
