...
function playLevel(game) {
  ...
  setTimeout(() => pinkDiagonal(650, 100, -5, 1, 'linear', 'circle'), 1000);
  setTimeout(() => pinkDiagonal(-50, 100, 5, 1, 'linear', 'circle'), 5000);
  setTimeout(() => blue(), 12000);
  setTimeout(() => green(), 13500);
}

function pinkDiagonal(x, y, velX, velY, move, attack) {
  let i = 0;
  let id = setInterval( () => {
