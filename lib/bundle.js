...
var EnemyShip = function (_MovingObject) {
  ...
  _createClass(EnemyShip, [{
    ...
    key: 'easeToY2',
    value: function easeToY2(y, ticks) {
      var _this2 = this;
    ...
    value: function easeToX(x, ticks) {
      ...
      if (this.isTweening === 'alive') {
        ...
        createjs.Tween.get(this.enemy, { override: true, useTicks: true }).wait(5).to({ x: x }, 45, createjs.Ease.backOut).call(function () {
          _this4.attack();
          setTimeout(function () {
            return _this4.isTweening = 'alive';
          }, 1500);
        });
      }
      return false;
    ...
    value: function multipleCircleAttack() {
      ...
      var id = setInterval(function () {
        for (var i = 0; i < 50; i++) {
          ...
          var x = 3 * Math.cos(r);
          var y = 3 * Math.sin(r);
          var bullet = new _bullets.purpleBullet(x, y, _this6.game);
          bullet.shoot(_this6.enemy.x, _this6.enemy.y);
          _this6.game.enemyBullets.push(bullet);
        }
        j += 1;
        if (j > 3 || _this6.health <= 0) {
          clearInterval(id);
        }
      }, 400);
    }
