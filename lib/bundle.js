...
var _boss = __webpack_require__(3);
...
function waveOne(game) {
  ...
  setTimeout(function () {
    return threePink(300, 0, 0.5, 1, 'ease2', 'single', game);
  }, 20000);
  setTimeout(function () {
...
function waveTwo(game) {
  ...
  setTimeout(function () {
    return purple(-50, 160, 0.5, 1, 'ease', 'multiple circles', game);
  }, 27000);
  setTimeout(function () {
    return threePink(300, -50, 0.5, 1, 'ease2', 'single', game, true);
  }, 30000);
...
function pinkDiagonal(x, y, velX, velY, move, attack, game) {
  var i = 0;
  var id = setInterval(function () {
    var pink = new _aliens.PinkAlien(velX, velY, game, 3, move, attack);
    pink.initiate(x, y);
    i += 1;
    if (i >= 10) {
      clearInterval(id);
...
var WAVES = [_level.waveOne, _level.waveTwo, _level.bossWave];
    ...
    value: function play() {
      ...
      this.ticker.addEventListener('tick', function () {
        if (!_this2.inProgress) {
          ...
          if (_this2.wave < 2) _this2.wave += 1;
        }
        if (_this2.isOver) {
