...
export class EnemyShip extends MovingObject {
  ...
  easeToY(y, ticks) {
    if (this.isTweening) {
      ...
      createjs.Tween.get(this.enemy, { override: true, useTicks: true, ignoreGlobalPause: true })
        .wait(5).to({ y: y }, 30, createjs.Ease.backOut)
        .call(() => {
          this.attack();
        });
    }
  ...
  easeToX(x, ticks) {
    if (this.isTweening) {
      ...
      this.isTweening = true;
      createjs.Tween.get(this.enemy, { override: true, useTicks: true, ignoreGlobalPause: true })
        .wait(5).to({ x: x }, 30, createjs.Ease.backOut)
        .call(() => {
          this.attack();
        });
    }
