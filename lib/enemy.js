...
import { pinkBullet, blueBullet, greenBullet } from './bullets';
...
export class EnemyShip extends MovingObject {
  ...
  easeTo(x, y, ticks) {
    if (this.isTweening) {
      ...
      createjs.Tween.get(this.enemy, { override: true, useTicks: true, ignoreGlobalPause: true }).wait(5).to({ y: y }, 30, createjs.Ease.backOut)
        .call(() => {
          this.attack();
        });
    }
